...
**Root Cause**: Some agent frameworks (e.g., Clawdbot) call `.trim()` on values that can be `undefined` during message streaming. This happens when:
...
**Fix**:
- Use `SIGUSR1` for manual restarts only when `commands.restart` is enabled
...
*Based on production deployment with Clawdbot agent framework + Claude Code*
