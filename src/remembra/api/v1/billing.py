...
async def _apply_paddle_refund(request: Request, meter: Any, result: Any) -> str:
    ...
    if subscription_id and not await _cancel_refunded_subscription(str(subscription_id), user_id):
        flag += "_cancel_failed"
        event = f"paddle_refund_cancel_failed:{subscription_id}"
        message += " The subscription could not be cancelled at Paddle: cancel it there before it renews and charges again."
    await _flag_account(
        request,
        meter,
        user_id,
        flag,
        message,
        {"subscription_id": subscription_id, "adjustment_id": result.transaction_id},
        event=event,
    )
