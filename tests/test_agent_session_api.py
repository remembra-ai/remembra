...
async def test_all_duplicate_store_sets_duplicate_of(env):
    ...
    first = await service.store(StoreRequest(content="Alice works at Acme", user_id="default_user"))
    assert first.status == "stored"

    class MatchQdrant(FakeQdrant):
        async def search(self, **kwargs: Any) -> list[Any]:
            return [(first.id, 0.97, {"content": "Alice works at Acme"})]
    ...
    assert resp.duplicate_of == first.id
