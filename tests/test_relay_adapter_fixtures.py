...
def test_codex_close_routes_a_usage_limit_stop_to_the_quota_path(tmp_path, dry_close):
    ...
    # The brief and the trail headline flag it like a Claude Code StopFailure.
    assert end_reason_note(body["end_reason"]) == "stopped: usage_limit"
    assert end_reason_note("other") is None
...
def test_timeouts_are_written_in_each_agents_unit(tmp_path):
    ...
    assert timeouts("claude-code") == {"SessionStart": 15, "SessionEnd": 15, "StopFailure": 15, "PreCompact": 15}
